<script lang="ts">
	...
	const incomingTickets = $derived(data.tickets);
	const incomingBranchId = $derived(data.selectedBranchId);
	const incomingStationId = $derived(data.selectedStationId);
	const wsToken = $derived(data.wsToken);
	const stations = $derived(data.stations as KitchenStation[]);
	...
	function connectWebSocket() {
		if (!branchId || !wsToken) return;
		const nextSocket = new WebSocket(buildWebSocketURL(branchId, wsToken));
		socket = nextSocket;
		isConnected = false;
...
<div class="max-w-7xl mx-auto space-y-6">
	<div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
		...
		<div class="text-xs text-gray-400">
			{#if isConnected}
				<span
					class="inline-flex items-center gap-1 rounded-full bg-green-50 px-2.5 py-1 text-green-600"
					>● Realtime aktif</span
				>
			{:else}
				<span
					class="inline-flex items-center gap-1 rounded-full bg-yellow-50 px-2.5 py-1 text-yellow-700"
					>● Menyambung...</span
				>
			{/if}
		</div>
	...
	{#if form?.success}
		<div class="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
			Perubahan KDS berhasil disimpan.
		</div>
	{/if}
	...
	<div class="grid gap-4 lg:grid-cols-[2fr,1fr]">
		<div class="rounded-xl bg-white p-4 shadow-sm">
			<div class="grid gap-3 md:grid-cols-[1fr,1fr,auto] md:items-end">
				<div>
					<label for="branch-filter" class="mb-1 block text-xs font-medium text-gray-500"
						>Cabang</label
					>
					<select
						id="branch-filter"
				...
				<div>
					<label for="station-filter" class="mb-1 block text-xs font-medium text-gray-500"
						>Station</label
					>
					<select
						id="station-filter"
		...
		<div class="rounded-xl bg-white p-4 shadow-sm">
			...
			>
				...
				<div>
					<label for="station-name" class="mb-1 block text-xs font-medium text-gray-500"
						>Nama Station</label
					>
					<input
						id="station-name"
						name="name"
						type="text"
						placeholder="Contoh: Grill"
						class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
					/>
				</div>
				<button
					type="submit"
					class="w-full rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:bg-gray-300"
					disabled={!branchId}>Buat Station</button
				>
			</form>
		</div>
	</div>
			...
			<div class="rounded-xl bg-white p-4 shadow-sm">
				<div class="mb-4 flex items-center justify-between">
					...
					<span class="rounded-full bg-yellow-50 px-2 py-0.5 text-xs font-medium text-yellow-700"
						>{ticketColumns.queued.length}</span
					>
				</div>
				<div class="space-y-3">
					{#if ticketColumns.queued.length === 0}
						...
						>
							Belum ada ticket baru.
						</p>
					{:else}
						{#each ticketColumns.queued as ticket (ticket.id)}
							<div
								class={`rounded-xl border p-4 transition ${isHighlighted(ticket.id) ? 'border-orange-300 bg-orange-50 shadow-md' : 'border-gray-100 bg-gray-50'}`}
							>
								<div class="mb-2 flex items-start justify-between gap-3">
									<div>
										<p class="font-semibold text-gray-800">{ticket.order_code}</p>
										<p class="text-xs text-gray-400">
											{stationLabel(ticket)}{#if ticket.table_id}
												· Meja #{ticket.table_id}{/if}
										</p>
									</div>
									<span
										class={`rounded-full px-2 py-0.5 text-xs font-medium ${statusMeta[ticket.status].badge}`}
										>{statusMeta[ticket.status].label}</span
									>
								</div>
								<p class="text-sm font-medium text-gray-700">{ticket.item_name}</p>
								<p class="mt-1 text-xs text-gray-500">
									Qty {ticket.quantity} · Masuk {formatTime(ticket.created_at)}
								</p>
								{#if ticket.notes}
									<p class="mt-2 rounded-lg bg-white px-3 py-2 text-xs italic text-gray-500">
										{ticket.notes}
									</p>
								{/if}
								<form
								...
								>
									...
									<button
										type="submit"
										class="w-full rounded-lg bg-orange-500 px-3 py-2 text-xs font-semibold text-white hover:bg-orange-600"
										>Mulai Proses</button
									>
								</form>
							</div>
			...
			<div class="rounded-xl bg-white p-4 shadow-sm">
				<div class="mb-4 flex items-center justify-between">
					...
					<span class="rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700"
						>{ticketColumns.inProgress.length}</span
					>
				</div>
				<div class="space-y-3">
							...
							>
								<div class="mb-2 flex items-start justify-between gap-3">
									<div>
										<p class="font-semibold text-gray-800">{ticket.order_code}</p>
										<p class="text-xs text-gray-400">
											{stationLabel(ticket)} · Mulai {formatTime(ticket.started_at)}
										</p>
									</div>
									<span
										class={`rounded-full px-2 py-0.5 text-xs font-medium ${statusMeta[ticket.status].badge}`}
			...
			<div class="rounded-xl bg-white p-4 shadow-sm">
				<div class="mb-4 flex items-center justify-between">
					...
					<span class="rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-green-700"
						>{ticketColumns.done.length}</span
					>
				</div>
				<div class="space-y-3">
							...
							>
								<div class="mb-2 flex items-start justify-between gap-3">
									<div>
										<p class="font-semibold text-gray-800">{ticket.order_code}</p>
										<p class="text-xs text-gray-400">
											{stationLabel(ticket)} · Selesai {formatTime(ticket.completed_at)}
										</p>
									</div>
									<span
										class={`rounded-full px-2 py-0.5 text-xs font-medium ${statusMeta[ticket.status].badge}`}
